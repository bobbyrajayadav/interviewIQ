...
function Step3Report({report}) {
  ...
  const downloadPDF = ()=>{
   ...
   // ==============SKILLS BOX================
    ...
    // ===================ADVICE================
    let advice ="Professional Advice";
    ...
    doc.setDrawColor(220);
        // 1. Increase the box height from 35 to 40 to fit the heading
    doc.roundedRect(margin, currentY, contentWidth, 40, 4, 4, "F");

    // 2. THIS IS THE MISSING HEADING CODE:
    doc.setFont("helvetica", "bold");
    doc.setFontSize(13);
    doc.setTextColor(0, 0, 0);
    doc.text("Professional Advice", margin + 10, currentY + 12);

    // 3. Reset the font back to normal for your advice text below
    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);
    doc.setTextColor(50, 50, 50);

    const splitAdvice = doc.splitTextToSize(advice, contentWidth - 20);
    // 4. Push the text down slightly from currentY + 20 to currentY + 22
  ...
  return (
    <div className='min-h-screen bg-linear-to-br from-gray-50 to-green-50 px-4 sm:px-6 lg:px-10 py-8'>
      <div className='mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
        <div className='md:mb-10 w-full flex items-center gap-4 flex-wrap'>
                  <button 
                  ...
                  className='mt-1 p-3 rounded-full bg-white shadow hover:shadow-md transition cursor-pointer'>
                    ...
                    <div>
                      <h1 className='text-3xl flex-nowrap font-bold text-gray-800 '>Interview Analytical Dashboard</h1>
          ...
          <button 
          onClick={downloadPDF}
          className='bg-emerald-600 hover:bg-emerald-700 text-white px-3 rounded-xl shadow-mg transition-all duration-300 font-semibold text-sm sm:text-base text-nowrap cursor-pointer'>Download PDF</button>
    ...
    {/* left side */}
        ...
        className='bg-white rounded-2xl sm:rounded-3xl shadow-lg p-6 sm:p-8 text-center'>
          ...
          <div className='relative w-20 h-20 sm:w-25 sm:h-25 mx-auto'>
              ...
              <p className='text-gray-400 mt-3 text-sm sm:text-base'>Out of 10</p>
              <div className='mt-4'>
                <p className='font-semibold text-gray-800 text-sm sm:text-base'>{performanceText}</p>
                <p className='text-gray-400 mt-1 text-sm sm:text-base'>{shortTagLine}</p>
              </div>
        </motion.div>
        ...
        className='bg-white rounded-2xl sm:rounded-3xl shadow-lg p-6 sm:p-8'>
          <h3 className='text-base sm:text-lg font-bold text-gray-700 mb-6'>
            Skill Evaluation
          </h3>
              ...
              className='bg-gray-50 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-200'> 
                <div className='flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-4'>
                  <div>
                    ...
                    <p className='text-xs textgray400'>
                      Questions {i + 1}
                    </p>
